...
scriptname = "get_certificate_details_from_file"
...
def getCertDetails(threadname,connection,domain,runversion):
    ...
    if can_read_certfile == 1:
        ...
        try:
            ...
            has_expire = 'Other'
            with open(logfile, 'a') as log:
                log.write('Error, ' + time.strftime("%Y-%m-%d %H:%M:%S") + ', ' + str(threadname) + ': Problem calculating expirde-time for Certfile: ' + str(certfile) + ' for domain: ' + str(domain) + '\n')
        ...
        i_duration = int(not_after[:-9])-int(not_before[:-9])

        years = i_duration / 100
        m_duration = i_duration%100
